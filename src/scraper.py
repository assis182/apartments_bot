...
class Yad2Scraper:
    ...
    def _setup_session(self):
        ...
        self.session.headers.update({
            ...
            'Cookie': '_ga=GA1.1.123456789.1234567890; _ga_XXXXXXXXXX=GS1.1.1234567890.1.1.1234567890.0.0.0',  # Add some common cookies
        })
    ...
    def _simulate_browser_behavior(self):
        ...
        try:
            # Visit the main site first with proper referrer
            ...
            # Visit the real estate section with proper referrer
            logger.debug("Visiting real estate section")
            realestate_response = self.session.get(
                f"{self.config.API_URL}/realestate",
                headers={
                    **self.session.headers,
                    'Referer': self.config.API_URL,
            ...
            # Visit the rent section with proper referrer
            logger.debug("Visiting rent section")
            rent_response = self.session.get(
                f"{self.config.API_URL}/realestate/rent",
                headers={
                    **self.session.headers,
                    'Referer': f"{self.config.API_URL}/realestate",
                    'Sec-Fetch-Site': 'same-origin',
                    'Sec-Fetch-Mode': 'navigate',
                    'Sec-Fetch-User': '?1',
                    'Sec-Fetch-Dest': 'document',
            ...
            all_cookies = {}
            for response in [main_response, realestate_response, rent_response]:
                if 'Set-Cookie' in response.headers:
                    logger.debug(f"Received cookies: {response.headers['Set-Cookie']}")
            ...
            if all_cookies:
                ...
                logger.debug(f"Updated session cookies: {cookie_string}")

        except Exception as e:
            logger.error(f"Error during browser simulation: {str(e)}")
            # Continue anyway - we might still succeed with the search
                ...
                for retry in range(max_retries):
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if 'ShieldSquare Captcha' in html_content or 'captcha' in html_content.lower():
                                logger.warning(f"Got captcha page on try {retry + 1}/{max_retries}")
                                if retry < max_retries - 1:
                                    continue  # Try again
                                else:
                                    logger.error(f"Failed all {max_retries} attempts for {neighborhood_name}")
                                    break  # Move to next neighborhood
